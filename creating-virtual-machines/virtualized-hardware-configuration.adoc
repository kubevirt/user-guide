...
This feature-gate allows kubevirt to take VM cpu model and create node selector from it.
With this node selector, VM can be scheduled on the node which can support VM cpu model.
Following VM has CPU model `Conroe`:
...
When CPUNodeDiscovery feature-gate is enabled, Kubevirt creates node selector with format:
...
To properly label the node, user can use 
https://github.com/ksimon1/cpu-node-labeller[cpu-node-labeller] in combination with
https://github.com/ksimon1/cpu-model-nfd-plugin[cpu-model-nfd-plugin] or create node labels by himself. 

To install cpu-node-labeller to cluster, run `oc apply -f cpu-node-labeller.yaml` with 
https://github.com/ksimon1/cpu-node-labeller/blob/master/cpu-node-labeller.yaml[this deploy manifest]
It will deploy cpu-node-labeller with cpu-model-nfd-plugin. 

cpu-model-nfd-plugin uses libvirt to get all supported cpu models on host and 
cpu-node-labeller create labels from cpu models. Then Kubevirt can schedule VM 
on node which has support for VM cpu model.
...
When CPUNodeDiscovery feature gate is enabled and nodes have labels with cpu models (as mentioned 
in link:#Enabling cpu compatibility enforcement[*Enabling cpu compatibility enforcement*]), Kubevirt creates node selector with default cpu model.
