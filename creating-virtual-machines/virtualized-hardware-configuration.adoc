...
[NOTE]
====
* Be sure that node CPU model where you run a VM, has the same or
higher CPU family.

* If CPU model wasn’t defined, the VM will have CPU model closest
to one that used on the node where the VM is running.

* CPU model is case sensitive.
====
...
[NOTE]
====
* Policy attribute can either be omitted or contain one of the following policies: force, require, optional, disable, forbid.
* In case a policy is omitted for a feature, it will default to *require*.
====
...
NOTE: Some guest operating systems or user payloads may require the RNG
device with enough entropy and may fail to boot without it. For example, fresh
Fedora images with newer kernels (4.16.4+) may require the `virtio` RNG device
to be present to boot to login.
