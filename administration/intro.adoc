...
needs to be added prior KubeVirt deployment when using the stable deployment flow:
____________________________________
Important: Before v0.15.0 release, please replace `kubevirt-handler` with
`kubevirt-privileged`.
____________________________________
[source,bash]
----
...
When using operator flow, SCC rules are created automatically.

Once privileges are granted, KubeVirt manifests can be deployed in the same way as explained above.
