...
The recommended way to deploy KubeVirt is using the KubeVirt Operator and
...
Since version 0.14.0, you can use `kubectl` for waiting until all KubeVirt components are "Ready".
...
2. Obsolete flow (manual deployment)
++++++++++++++++++++++++++++++++++++

Before operator, one would install KubeVirt using a manifest file, which
contains all deployments and configurations of the KubeVirt core components:
