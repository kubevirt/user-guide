...
Zero downtime rolling updates supported from the release `v0.17.0`:
 
Starting in `v0.17.0`, the KubeVirt operator supports updates. Updates are
triggered by changing the imageTag value in the KubeVirt CR's spec.
...
NOTE: Updating from any release prior to the KubeVirt `v0.17.x` series is not
supported.
