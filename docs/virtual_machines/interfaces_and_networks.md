...
##### Customizing the Multus pod annotation

It's possible to customize the Multus pod annotation `k8s.v1.cni.cncf.io/networks` by adding the 
`k8s.v1.cni.cncf.io/networks-customization` annotation to your `VirtualMachineInstance` specification. 
This annotation will be merged with the one generated by KubeVirt.

The `k8s.v1.cni.cncf.io/networks-customization` annotation has the 
[same contract](https://github.com/k8snetworkplumbingwg/multus-cni/blob/master/pkg/types/types.go#L128) and should be a 
JSON array, where each object in the array represents a network interface. Two fields, managed by KubeVirt, can't
be changed through this customization annotation:
- `interface`: already resolved by the KubeVirt's virt-controller when the Multus pod annotation is generated.
- `mac`: can already be provided through the VMI spec (`interfaces.*.macAddress`)

Here's an example of how to customize the Multus pod annotation:
```yaml
kind: VirtualMachine
metadata:
  name: vm
spec:
  template:
    metadata:
      annotations:
        k8s.v1.cni.cncf.io/networks-customization: '[{"name": "network1", "namespace": "default", "ips":["10.100.2.102/24"]}]'
```
This VM definition would generate a pod definition as below:
```yaml
kind: Pod
metadata:
  name: virt-launcher-vm
  annotations:
    k8s.v1.cni.cncf.io/networks: '[{"name":"network1","namespace":"default","interface":"pod890360ec7a1", "ips":["10.100.2.102/24"]}]'
    k8s.v1.cni.cncf.io/networks-customization": '[{"name": "network1", "namespace": "default", "ips":["10.100.2.102/24"]}]'
```
