...
1. `istio-proxy` sidecar is not deployed in VMI virt-launcher pod:
...
2. `istio-proxy` sidecar is not Ready:
```shell
$ kubectl get pods
NAME                             READY   STATUS    RESTARTS   AGE
local-volume-provisioner-hxzng   1/1     Running   0          13h
local-volume-provisioner-kfkbk   1/1     Running   0          13h
local-volume-provisioner-qb9fz   1/1     Running   0          13h
...
3. Virt-launcher pod for VMI is stuck at `Init:0/3` status:
```shell
$ kubectl get pods
NAME                             READY   STATUS     RESTARTS   AGE
