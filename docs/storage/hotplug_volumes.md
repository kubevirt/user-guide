...
The scsi bus should be used if a large number of disks will be hotplugged concurrently. There are a limited number of virtio ports available. The scsi bus has [very close in performance](https://mpolednik.github.io/2017/01/23/virtio-blk-vs-virtio-scsi/) to virtio.
...
spec:
  ...
  template:
    spec:
      ...
      volumes:
      - containerDisk:
          image: quay.io/kubevirt/alpine-container-disk-demo:20250701_e93e609e3d
        name: disk0
```
...
spec:
  source:
    registry:
      url: "docker://quay.io/kubevirt/alpine-container-disk-demo:20250701_e93e609e3d"
  storage:
    resources:
      requests:
        storage: 300Mi
...
This operation will fail if the VMI is owned by a VirtualMachine
