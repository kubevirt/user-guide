...
There is a particular race condition which might occur during the migration which needs to be carefully verified before cancelling the update.
...
```
  2. Libvirt reported the domain from paused to running in the target virt-launcher pod and KubeVirt sets when the domain started on the target:
```bash
$ kubectl get vmi vmi-migratable -ojsonpath='{.status.migrationState.targetNodeDomainReadyTimestamp}'
2025-01-14T08:29:07Z
```

In this case, the correct volumes are the destination volumes and the last writes to the storage could have been performed on the destination volumes rather then the original ones.
