...
The `spec.configuration.instancetype.referencePolicy` configurable can then be used to configure one of the above policies:

```sh
kubectl patch -n kubevirt kv/kubevirt --type merge -p '{"spec":{"configuration":{"instancetype":{"referencePolicy": "expand"}}}}'
```

As outlined above, the `expand` policy results in any instance type or preference referenced by the VirtualMachine being expanded in the resulting object:
