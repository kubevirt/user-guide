...
- Make sure the [common-instancetypes](https://kubevirt.io/user-guide/user_workloads/deploy_common_instancetypes/)
  are deployed.
...
- If the custom kernel was configured to build modules, we need to install these in the root file system. For this
  example the kernel modules where tar'ed and packaged into an isofs. This isofs is then added to the `containerDisk`
  and unpacked into the root file system with the cloudinit runcmd.
