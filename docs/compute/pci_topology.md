...
and [hotplugging network interfaces](../network/hotplug/interfaces.md) after
...
| Topology Version | KubeVirt Versions |
|:----------------:|:-----------------:|
| v1               | < 1.6             |
| v2               | 1.6, 1.7 (before 1.6.4 / 1.7.2) |
| v3               | 1.6.4+, 1.7.2+, 1.8+ |

### v1: Fixed Placeholders

The original implementation used `max(0, 4 - interfaces)` placeholders, providing
...
### v3: Fixed Placeholders + Extra Controllers
...
As described in [PCI Topology Versions](#pci-topology-versions), VMs created
under different topology versions use different port reservation strategies.
When upgrading KubeVirt, the topology version may change, which can affect
device PCI addresses.
...
#### Running VMs (automatically preserves addresses)
...
### Windows VM Disks Appear Offline After Upgrade
